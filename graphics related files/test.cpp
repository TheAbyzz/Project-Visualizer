...
int main(){
    ...
    uniform_int_distribution<mt19937::result_type> distr1(0,1); // 50,80

    // index of vertex (point)
    random_device indx_vertex_seed;
    mt19937 engine2(indx_vertex_seed());
    uniform_int_distribution<mt19937::result_type> distr2(0,499);
    ...
    double K = sqrt((1500*1500)/graph_vertices.size()); //0.5* or 0.000005
    ...
    for(auto i = 0; i < 100; i++){
        ...
        for(auto j = 0; j < graph_vertices.size(); j++){
            ...
            // scaling x and y
            //trans_point1[0] *= 0.5;        //0.9;
            //trans_point1[1] *= 0.5;        //0.9;
            ...
            for(auto k = 0; k < graph_vertices[j].neighbours.size(); k++){
                ...
                // scaling x and y
                //trans_point2[0] *= 0.5;       //0.9;
                //trans_point2[1] *= 0.5;       //0.9;
        ...
        for(auto j = 0; j < graph_vertices.size(); j++){
            ...
            if(pow(graph_vertices[j].n_cords[0],2)+pow(graph_vertices[j].n_cords[1],2)+pow(graph_vertices[j].n_cords[2],2) > 0.85){ // 0.5
                double mag = sqrt(pow(graph_vertices[j].n_cords[0],2)+pow(graph_vertices[j].n_cords[1],2)+pow(graph_vertices[j].n_cords[2],2));

                graph_vertices[j].n_cords[0] = ((double)0.85/mag)*graph_vertices[j].n_cords[0];
                graph_vertices[j].n_cords[1] = ((double)0.85/mag)*graph_vertices[j].n_cords[1];
                graph_vertices[j].n_cords[2] = ((double)0.85/mag)*graph_vertices[j].n_cords[2];
            }
        }
    ...
    while(!dataReady.load()){
        ...
        for(auto j = 0; j < graph_vertices.size(); j++){
            ...
            cout << proj_point1[2] << endl;
            XDrawArc(disp, win, gc, round(proj_point1[0]*(double)1500), round(proj_point1[1]*(double)1500), 5*proj_point1[2], 5*proj_point1[2], 0, 360*64);
            XFlush(disp);            
        }
